...
from models import VendorContactPerson as Model
...
class Service:
    ...
    async def list(self, request: Request, page=1, size=10, search: str | None = None,time_period: TimePeriodEnum | None = None, start_date: date | None = None, end_date: date | None = None, ) -> ListSchema:
    ...
    async def create(self, request: Request, item: CreateSchema):
        """Create vendor contact person with validation"""
        # ✅ Validate input data
    ...
    async def update(self, request: Request, id: UUID, item: CreateSchema):
        """Update vendor contact person with validation"""
        user = request.state.user
        obj = await self.get_object(id)
