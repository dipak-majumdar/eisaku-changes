...
from models import District as Model
from schemas import (
...
OBJECT_NOT_FOUND = messages.DISTRICT_NOT_FOUND
OBJECT_EXIST = messages.DISTRICT_EXIST
OBJECT_DELETED = messages.DISTRICT_DELETED
...
class Service:
    ...
    async def list(self, request: Request, page=1, size=10, search: str | None = None, state_id: UUID | None = None) -> ListSchema:
        statement = select(Model)

        if state_id:
            statement = statement.where(Model.state_id == state_id)
