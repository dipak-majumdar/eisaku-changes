...
from models import State as Model
from schemas import (
    StateList as ListSchema, 
    StateRead as ReadSchema, 
    PermissionReadWithRequired
)


OBJECT_NOT_FOUND = messages.STATE_NOT_FOUND
OBJECT_EXIST = messages.STATE_EXIST
OBJECT_DELETED = messages.STATE_DELETED
...
class Service:
    ...
    async def list(self, request: Request, page=1, size=10, search: str | None = None, country_id: UUID | None = None) -> ListSchema:
        ...
        if country_id:
            statement = statement.where(Model.country_id == country_id)
    ...
    async def create(self, request: Request, item: Model):
        ...
        obj = Model(**item.dict(), created_by=user.id, updated_by=user.id)  # ✅ Convert to table model
    ...
    async def update(self, request: Request, id: UUID, item: Model):
        ...
        # ✅ Update only the updated_by, updated_at field
