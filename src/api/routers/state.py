...
from api.deps import get_state_service as get_service
from services import StateService as Service
from schemas import (
    StateRead as ReadSchema, 
    StateList as ListSchema, 
    StateCreate as CreateSchema, 
    StateUpdate as UpdateSchema
)

CREATE_PERMISSION = 'state.can_create'
CHANGE_PERMISSION = 'state.can_change'
VIEW_PERMISSION = 'state.can_view'
DELETE_PERMISSION = 'state.can_delete'
...
async def list(
    ...
    return await service.list(request, page=page, size=size, search=search, country_id=country_id)
