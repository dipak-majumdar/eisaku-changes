...
from api.deps import get_vendor_agreement_service as get_service
from models.enums import TimePeriodEnum
from services import VendorAgreementService as Service
from schemas import (
...
async def list(
    ...
    return await service.list(request, page=page, size=size, search=search, vendor_id=vendor_id, time_period=time_period, start_date=start_date, end_date=end_date)
...
async def create(
    request: Request,
    vendor_id: UUID = Form(..., description="Vendor UUID"),
